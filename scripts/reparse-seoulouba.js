...
(async () => {
  ...
  // A) 승인대기 (최근 수집 — 페이지 유효)
  const staged = (await db.execute("SELECT id, source_id, channel, days, hours, exclude_holiday FROM scraped_items WHERE platform='서울오빠' AND status='pending'")).rows;
  ...
  for (const r of staged) {
    try {
      ...
      const nd = d.days || r.days || '', nh = d.hours || r.hours || '', nc = d.channel || r.channel || '', eh = d.excludeHoliday || 0; // 새값 없으면 기존 유지
      if (nh === (r.hours || '') && nd === (r.days || '') && nc === (r.channel || '') && eh === (r.exclude_holiday || 0)) { same++; continue; }
      chg++; if (printed++ < 15) console.log(`  #${r.source_id} 요일[${r.days}→${nd}] 시간["${r.hours}"→"${nh}"] 채널[${r.channel}→${nc}]`);
      if (APPLY) await db.execute({ sql: 'UPDATE scraped_items SET days=?, hours=?, channel=?, exclude_holiday=? WHERE id=?', args: [nd, nh, nc, eh, r.id] });
    } catch (e) { fail++; }
  }
  // B) 등록 캠페인 — 활성(마감 미래/상시)만. 만료는 지도에 안 나오고 페이지도 바뀌어 위험.
  const camps = (await db.execute({ sql: "SELECT id, link, channels, operating_days, operating_hours, exclude_holiday FROM campaigns WHERE link LIKE '%seoulouba%' AND (deadline='' OR deadline IS NULL OR deadline >= ?)", args: [today] })).rows;
  ...
  for (const r of camps) {
    ...
    try {
      ...
      if (newHours === (r.operating_hours || '') && newDays === (r.operating_days || '[]') && newCh === (r.channels || '[]') && eh === (r.exclude_holiday || 0)) { same++; continue; }
      chg++; if (printed++ < 25) console.log(`  camp#${r.id} 요일[${r.operating_days}→${newDays}] 시간["${r.operating_hours}"→"${newHours}"] 채널[${r.channels}→${newCh}]`);
      if (APPLY) await db.execute({ sql: 'UPDATE campaigns SET operating_days=?, operating_hours=?, channels=?, exclude_holiday=? WHERE id=?', args: [newDays, newHours, newCh, eh, r.id] });
    } catch (e) { fail++; }
  }
