...
function categoryByKeyword(content, name) {
  ...
  if (/네일|피부|왁싱|헤어|미용|에스테틱|태닝|속눈썹|반영구|필러|보톡스|두피|체형|다이어트|펌(?![프킨])|염색|풋앤바디|바디케어|스킨케어|피부관리|메이크업|브로우|눈썹|슈가링|제모|타투|\bSMP\b|두피채움|페디큐어|풋케어|발각질|내성발톱|발톱무좀|바버샵|이발|커트|샴푸|스타일링|여드름|아로마|헤드스파/.test(s)) return '뷰티';
