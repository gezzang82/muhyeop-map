...
function categoryByKeyword(content, name) {
  ...
  // 찜질방·사우나 → 기타(운동 판정 이후라 휘트니스&사우나는 운동 유지). 숙박(찜질방펜션 등)이면 아래 숙박 규칙에 양보.
  if (!/숙박|호텔|모텔|펜션|글램핑|카라반|풀빌라|리조트|게스트하우스|한옥스테이|캠핑/.test(s) && /찜질방|사우나/.test(s)) return '기타';
...
const CATEGORY_OVERRIDE = [
  ...
  [/에뚜왈\s*가로수길점/, '카페'],
  [/부타캣\s*강서점/, '문화'],
  [/마라공방/, '음식점'],   // '공방'이 문화로 오검출되던 마라탕 체인
  [/제이델링/, '카페'],     // 디저트/델리 카페 체인(음식점 오검출 방지)
];
