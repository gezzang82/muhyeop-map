...
function categoryByKeyword(content, name) {
  ...
  if (/헬스|피트니스|필라테스|요가|골프|스크린골프|클라이밍|크로스핏|복싱|주짓수|테니스|스쿼시|\bPT\b|퍼스널\s*트레이닝|무에타이|킥복싱|펜싱|볼링|당구|다이빙|서핑|스케이트|양궁|크루즈|유람선|요트|배낚시|선상낚시|실내낚시|좌대낚시|바다낚시|낚시터|밤낚시|키즈수영|수영강습|수영\s*특강|수영교실|유아수영|아기수영|생존수영/.test(s)) return '기타';
