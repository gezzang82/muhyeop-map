...
function categoryByKeyword(content, name) {
  ...
  if (/헬스|스포애니|피트니스|휘트니스|운동\s*센터|운동\s*클럽|필라테스|요가|골프|스크린골프|클라이밍|크로스핏|복싱|주짓수|테니스|스쿼시|\bPT\b|퍼스널\s*트레이닝|무에타이|킥복싱|펜싱|스피닝|점핑\s*다이어트|웨이트\s*트레이닝|축구|풋살|농구|배드민턴|탁구|(?<!이삿)(?<!이사)(?<!화물)짐(?![가-힣])/.test(s)) return '운동';
  ...
  if (/세차|세차장|손세차|셀프세차|디테일링|광택|세탁|빨래|드라이클리닝|코인빨래|코인워시|애견|반려동물|반려견|반려묘|퍼피|펫샵|펫호텔|펫카페|펫막스|펫스토어|반려용품|\b펫\b|강아지|고양이|동물병원|파충류|파충류샵|게코샵|게코|이구아나|비어디드래곤|렙타일|수선|열쇠|철물|프린트|인쇄|택배|무인점포|무인매장|키즈카페|베이비카페|영유아카페|놀이방|피로연|예식장|웨딩홀/.test(s)) return '기타';
  ...
  if (/눈썹|브로우|왁싱|반영구|[Ss][Mm][Pp]\b|두피채움|속눈썹|네일/.test(s)) return '뷰티';
  ...
  if (/네일|피부|왁싱|헤어|미용|에스테틱|태닝|속눈썹|반영구|필러|보톡스|두피|체형|튼살|셀룰라이트|다이어트|펌(?![프킨])|염색|풋앤바디|바디케어|스킨케어|피부관리|메이크업|브로우|눈썹|슈가링|제모|타투|\bSMP\b|두피채움|페디큐어|풋케어|발각질|내성발톱|발톱무좀|바버샵|이발|커트|샴푸|스타일링|여드름|아로마|헤드스파/.test(s)) return '뷰티';
  ...
  if (/맞춤정장|맞춤셔츠|와이셔츠|드레스셔츠|정장|양복|한복|교복|실내복|실외복|아동복|유아복|원피스|니트\b|코트\b|자켓|재킷|가디건|맨투맨|후드티|청바지|데님|의류|구제샵|구제옷|편집샵|스트릿웨어/.test(s)) return '의류';
...
function mapCategory(platformCat, content, name) {
  ...
  if (/헬스|스포애니|피트니스|휘트니스|운동\s*센터|운동\s*클럽|필라테스|요가|골프|스크린골프|클라이밍|크로스핏|복싱|주짓수|테니스|스쿼시|\bPT\b|퍼스널\s*트레이닝|무에타이|킥복싱|펜싱|스피닝|웨이트\s*트레이닝|축구|풋살|농구|배드민턴|탁구|(?<!이삿)(?<!이사)(?<!화물)짐(?![가-힣])/i.test(s)) return { cat: '운동', flag: false };
  ...
  if (platformCat === '여가') {
    ...
    if (/헬스|스포애니|피트니스|휘트니스|운동\s*센터|운동\s*클럽|필라테스|요가|골프|클라이밍|스크린골프|퍼스널트레이닝|\bPT\b/i.test(s)) return { cat: '운동', flag: false };
...
function soName(html) {
  ...
  // 이름 끝 노트 괄호 제거: "경성빵공장 남한산성점 (테이크아웃 가능)" → 매장명만. (지점명 "(강남점)" 등은 노트어가 없어 보존)
  name = name.replace(/\s*\((?:[^)]*(?:테이크아웃|포장|예약|주차|가능|필수|불가)[^)]*)\)\s*$/, '').trim();
  return { name, channel };
}
...
async function rnFetchPage(page, city) {
  // v2 공개 API가 실제 페이지네이션 + `city=`(시/도) 지역필터를 지원. (SSG `/campaigns` 페이지는 page/필터를
  //  무시하고 빌드시점 96건만 고정 반환 → 전체 접근 불가였음. v2는 무인증으로 지역별 전량 페이징.)
  const cityQ = city ? `&city=${encodeURIComponent(city)}` : '';
  const res = await fetch(`${RN_BASE}/api/v2/campaigns?page=${page}&size=20${cityQ}`, { headers: { 'User-Agent': UA, Accept: 'application/json', Referer: `${RN_BASE}/campaigns` } });
...
async function runReviewnote({ db, limit = 300, deadlineTs = 0, region = '', dedupe: _dedupe = null }) {
  ...
  // 작고 안정적이라(랭킹 변동 영향↓) 디너의여왕처럼 체계적으로 완주. region 없으면 전국 통합(레거시).
  const stateKey = region ? `리뷰노트:${region}` : '리뷰노트';
  const stRes = await db.execute({ sql: 'SELECT last_max_id FROM scrape_state WHERE platform = ?', args: [stateKey] });
  const startPage = Math.max(1, Number(stRes.rows[0]?.last_max_id || 0) || 1);
  const MAX_PAGES = region ? 12 : 40; // 지역별은 17개 순회라 패스당 12페이지(≈190건)씩; 전국모드는 40
  ...
  for (; pagesThis < MAX_PAGES; pagesThis++, page++) {
    ...
    let data; try { data = await rnFetchPage(page, region); } catch (e) { stopped = true; break; }
  ...
  await db.execute({
    sql: `INSERT INTO scrape_state (platform, last_max_id, last_run_at) VALUES (?, ?, datetime('now','+9 hours'))
          ON CONFLICT(platform) DO UPDATE SET last_max_id = excluded.last_max_id, last_run_at = excluded.last_run_at`,
    args: [stateKey, nextPage],
  });
  await db.execute({
    sql: `INSERT INTO scrape_runs (platform, cursor_from, cursor_to, fetched, staged, excluded, note)
          VALUES ('리뷰노트', ?, ?, ?, ?, ?, ?)`,
    args: [startPage, nextPage, processed, staged, excluded + dupActive, `${region || '전국'} page ${startPage}~${page}${reachedEnd ? '(끝→1)' : ''} 처리 ${processed} (적재 ${staged}, dup_active ${dupActive}, 좌표실패 ${geoFail}, 제외 ${excluded}, 실패 ${failed}${timedOut ? ', 시간중단' : ''})`],
  });
