...
function categoryByKeyword(content, name) {
  ...
  if (/사주|타로|운세|점집|신점|철학관|작명|무속|명리|손금|관상|점사/.test(s)) return '기타';
  if (/헬스|피트니스|필라테스|요가|골프|스크린골프|클라이밍|크로스핏|복싱|주짓수|테니스|스쿼시|\bPT\b|퍼스널\s*트레이닝|무에타이|킥복싱|펜싱|볼링|당구|다이빙|서핑|스케이트|양궁|크루즈|유람선|요트|배낚시|선상낚시|실내낚시|좌대낚시|바다낚시|낚시터|밤낚시/.test(s)) return '기타';
  if (/세차|세차장|손세차|셀프세차|디테일링|광택|세탁|빨래|드라이클리닝|코인빨래|코인워시|애견|반려동물|반려견|반려묘|퍼피|펫샵|펫호텔|펫카페|펫막스|펫스토어|반려용품|\b펫\b|강아지|고양이|동물병원|파충류|파충류샵|게코샵|렙타일|수선|열쇠|철물|프린트|인쇄|택배|무인점포|무인매장/.test(s)) return '기타';
  if (/약손|안마|마사지|지압|경락|스웨디시|발마사지|타이마사지|풋마사지|스포츠마사지|림프|테라피|족욕|디톡스\s*족욕|웰리스|웰니스|상담센터|심리상담|전화상담|방문상담|발달센터|치유센터|금거래소|금은방|귀금속|결혼정보|소개팅|컨설팅|신당|신점|보살|운전면허/.test(s)) return '기타';
  ...
  if (/원데이\s*클래스|클래스\s*체험|보컬|레슨|트레이닝|학원|공방|드로잉|플라워|캔들|공예|만들기|전시|관람|원데이클래스|대관|스튜디오|셀프사진|포토부스|방탈출|보드게임|사진관|증명사진|프로필\s*촬영|사진\s*스튜디오|갤러리|공연|영화|VR|브이알|테마파크|미술관|박물관|만화카페|만화방|북카페/.test(s)) return '문화';
  ...
  if (/네일|피부|왁싱|헤어|미용|에스테틱|태닝|속눈썹|반영구|필러|보톡스|두피|체형|다이어트|\b펌\b|염색|풋앤바디|바디케어|스킨케어|피부관리|메이크업|브로우|슈가링|제모|타투|페디큐어|풋케어|발각질|내성발톱|발톱무좀/.test(s)) return '뷰티';
...
function mapCategory(platformCat, content, name) {
  ...
  if (/헬스|피트니스|필라테스|요가|골프|스크린골프|클라이밍|크로스핏|복싱|주짓수|테니스|스쿼시|\bPT\b|퍼스널\s*트레이닝|무에타이|킥복싱|펜싱|볼링|당구|다이빙|서핑|스피닝|크루즈|유람선|요트|족욕|점사/i.test(s)) return { cat: '기타', flag: false };
