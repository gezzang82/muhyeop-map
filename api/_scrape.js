...
function categoryByKeyword(content, name) {
  ...
  if (/헬스|피트니스|필라테스|요가|골프|스크린골프|클라이밍|크로스핏|복싱|주짓수|테니스|스쿼시|\bPT\b|퍼스널\s*트레이닝|무에타이|킥복싱|펜싱|볼링|당구|다이빙|서핑|스케이트|양궁/.test(s)) return '기타';
  if (/세차|세차장|손세차|셀프세차|디테일링|광택|세탁|빨래|드라이클리닝|코인빨래|코인워시|애견|반려동물|\b펫\b|강아지|고양이|동물병원|수선|열쇠|철물|프린트|인쇄|택배|무인점포|무인매장/.test(s)) return '기타';
