...
- **환경변수**: 위 Turso 2개 + `SESSION_SECRET`·`ADMIN_PASSWORD`·`PUBLIC_BASE_URL`·카카오/네이버 OAuth·`NAVER_SEARCH_*`. **오토파일럿용 추가**: `OPENAI_API_KEY`(AI 판정), `CRON_SECRET`(크론 인증 — 없으면 크론 401), 선택 `OPENAI_MODEL`(기본 gpt-4o-mini).
- **크론**: `vercel.json`에 매일 06:00 KST(21:00 UTC) `POST /api/campaigns?action=autopilot&scrape=dinnerqueen` 1건. `functions.maxDuration=300`(오토파일럿 시간예산 240s). 상세 `docs/product/03-platform-analysis.md` 8절.
...
- `api/_scrape.js` — 디너의여왕/포블로그 서버 사이드 증분 수집기(`_` 접두). `runScrape`/`runDinnerqueen`(커서 `scrape_state.last_max_id`, 결정적 파서, `classify`로 활성중복 제외·`dedupe_status` 부여)·`reparsePending`. `deadlineTs`로 시간예산 초과 시 중단(못한 신규는 커서가 다음 실행에서 이어받음). `campaigns.js`가 import.
...
- `api/_geocode.js` — 서버 사이드 지오코딩(`_` 접두). `geocodeServer`가 네이버 로컬 검색 API(`NAVER_SEARCH` 키 재사용)의 `mapx/mapy`(WGS84*10^7)로 좌표 획득, 한국 좌표 범위 밖이면 null(→검수큐). 크론엔 브라우저 geocode(`naver.maps.Service`)를 못 써서 도입.
