...
**로컬 크롤러(`scripts/crawl-worker.js`)로 단일화**. 컴퓨터가 켜진 동안 전 플랫폼(디너/강남/링블/포블로그/서울오빠/리뷰노트/오마이블로그/구구다스/라미라미/레뷰/포포몬)을 순회 수집→오토파일럿. 과거 매일 06:00 KST Vercel 크론(`POST ?action=autopilot&scrape=dinnerqueen`)은 **①디너의여왕만 커버 ②컴퓨터가 켜진 06:00엔 로컬 크롤러와 겹쳐 오토파일럿 중복(같은 매장 2중 INSERT) 위험**이라 **폐지**([[06-decision-log]] 2026-08-30). 수동 트리거(`POST /api/campaigns?action=autopilot`, 어드민 [지금 실행]/[미리보기] 버튼, 인증=관리자)는 유지. 한 패스 안에서:
...
경쟁사 리뷰노마드([11-competition](11-competition.md))가 **~27개 플랫폼**을 집계 → 무협맵이 안 다루는 플랫폼들의 **무인증 파싱 가능성을 전수 조사**. 기준: ①로그인 없이 목록 접근 가능한가(**A**=SSR/`__NEXT_DATA__`, **B**=공개 JSON API, **C**=인증필요·불가) ②방문형+위치(매장 지도라 필수) ③파싱 난이도.

**구현 현황(2026-09-20)**: 디너의여왕·강남맛집·리뷰노트·서울오빠·포블로그·링블 + 오마이블로그·구구다스·라미라미(회원초대)·**레뷰**·**포포몬** = 11개 러너 가동(`api/_scrape.js`, [[api-db]]). 아래 표의 🟢 후보 중 오마이블로그·구구다스·포포몬은 구현 완료.
- **레뷰**(revu.net, 대형): 무인증 공개 큐레이션은 ~40건뿐이라, **계정 로그인(인증) 크롤**로 전체 활성 카탈로그(~3천 방문형) 수집(백엔드 `api.weble.net`, JWT). 좌표는 제공되나 도로명 지오코딩으로 통일, 방문시간은 상세 `altVisitInfo`. 경쟁사 대부분이 크롤하는 흐름 + 링크중심(복제 아님)이라 착수([[06-decision-log]] 2026-09-19~20).
- **포포몬**(popomon.com): 무인증 공개 API(목록 `fetch_getcampaignlist?recruitType=visiting` + 상세 `fetch_getCampaignData`)로 방문형 ~3.5천 수집, 상세에 도로명주소·방문시간.
...
### 착수 우선순위 & 시사점
1. **최우선 4곳(공개API·주소/좌표 완비, 유지보수 최소)**: 오마이블로그 → 구구다스 → 스토리앤미디 → 리뷰플레이스. 좌표/전체주소가 있어 지오코딩 부담 적음. **✅ 오마이블로그·구구다스(2026-09-10) 구현 완료** — 구구다스는 목록 JSON(`cmpnList.do`, XHR헤더 필요)+상세 HTML(`매장주소:` 라벨·카카오맵 좌표·방문시간), 원고료형 스킵. 다음은 **스토리앤미디·리뷰플레이스**.
