...
## 매장/캠페인 등록 폼
- **직전 캠페인 프리필**(2026-07-31): 매장 콤보에서 **기존 매장을 직접 고르면**(`pickExistingPlaceForNew`) 그 매장의 **가장 최근 캠페인 값**(플랫폼/채널/내용/링크/운영시간/요일/공휴일)을 폼에 자동 채움 — **마감일만 새로 입력**하면 됨(`prefillCampaignFromLast`, 마감일은 안 건드림). 체험단 마감이 ~5일이라 같은 매장을 반복 등록하는 일상 작업을 단축. `editCampaign`은 `pickExistingPlace`를 직접 호출하므로 프리필 대상 아님(수정은 해당 캠페인 값으로 채움).
